...
struct PersistenceController {
    ...
    static var preview: PersistenceController = {
        ...
        for i in 0..<10 {
            ...
            let numberOfCards = Int.random(in: 0...6)
            for j in 0..<numberOfCards {
                let newCarte = Carte(context: viewContext)
                newCarte.avant = "Avant \(i) - \(j)"
                newCarte.arriere = "Arrière \(i) - \(j)"
                
                newCarte.dateProchaineRevision = Date()
    ...
    init(inMemory: Bool = false) {
        container = NSPersistentContainer(name: "LearnIO")
        if inMemory {
